...
def mask_to_image(mask: np.ndarray, mapping = {}):
    if mask.ndim == 2:
        ...
        # probabilities to indexes --> index of each class that has the highest probability
        ...
        pred_image = torch.zeros(3, mask.size(0), mask.size(1), dtype=torch.uint8)
        # replace predicted mask values with mapped values
        for k in rev_mapping:
            pred_image[:, mask == k] = torch.tensor(rev_mapping[k]).byte().view(3, 1)
        final_mask_pred = pred_image.permute(1, 2, 0).numpy()
        return Image.fromarray(final_mask_pred)
